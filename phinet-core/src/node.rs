...
impl PhiNode {
                                ...
                                RelayCommand::Extend2 => {
                                    ...
                                    match mgr.begin_extend(
                                        ...
                                        Ok(bytes) => {
                                            ...
                                            {
                                                warn!("extend: next hop {} not connected \
        ...
        -> Option<crate::wire::HsDescriptor>
    {
        ...
        };
        {
            ...
            info!("hs lookup {}: broadcasting to {} peer(s)",
    ...
    pub async fn hs_fetch(
        ...
        use crate::path_select::{select_path, PathError, SelectedPath};
        ...
        let rp_path = {
            ...
            for _ in 0..24 {
                let p = select_path(&mut rng, consensus, &[self_id.clone()], None)
                    .map_err(|PathError::InsufficientRelays(s)| Error::Handshake(
                        format!("hs_fetch: RP path selection: {s}")))?;
                if p.hops.last().map(|h| h.node_id_hex != hs_hex).unwrap_or(false) {
                    chosen = Some(p);
                    break;
                }
            }
            match chosen {
                ...
                None => return Err(Error::Handshake(
                    "hs_fetch: RP path selection: every draw put the rendezvous \
                     point on the service's own relay — the network needs more \
                     relays".into())),
            }
        };
        let rp_specs = rp_path.to_link_specs()
            .map_err(|e| Error::Handshake(format!("hs_fetch: RP link specs: {e:?}")))?;
        let rp_terminal = rp_specs.last()
            .ok_or_else(|| Error::Handshake("hs_fetch: empty RP path".into()))?
            .clone();
        let rp_cid = Arc::clone(&self).build_hs_circuit(rp_specs).await
        ...
        excluded.extend(rp_path.hops.iter().map(|h| h.node_id_hex.clone()));
    ...
    async fn maintain_guards_from_consensus(self: &Arc<Self>) {
        let me = self.node_id();
